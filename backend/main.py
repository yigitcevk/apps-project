...
@app.get("/assets", response_model=List[users.schemas.Asset], dependencies=[Depends(auth.verify_token)])
...
@app.get("/campaigns", response_model=List[users.schemas.Campaign], dependencies=[Depends(auth.verify_token)])
...
@app.get("/insights", response_model=List[users.schemas.Insight], dependencies=[Depends(auth.verify_token)])
async def get_insights(db: Session = Depends(get_db)):
    return db.query(users.models.Insight).all()

# Route to get all campaign_creatives stored in the list.
@app.get("/campaign_creatives", response_model=List[users.schemas.CampaignCreative], dependencies=[Depends(auth.verify_token)])
async def get_insights(db: Session = Depends(get_db)):
